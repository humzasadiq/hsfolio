...
const CardWithBorders = ({ title, description, technologies, link, github }) => {
  ...
  return (
    <>
      <div
        ref={ref}
        className={`flex items-center justify-center bg-[#F3F4F6] dark:bg-black w-80 h-56 ${
          isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"
        }`}
      >
      <div className="relative w-80 h-56 bg-F3F4F6 dark:bg-black rounded-md border border-blue-200 dark:border-gray-700">
        ...
        <div className="p-4 transition-all duration-500 delay-200">
          ...
          <div className="gap-2 my-1 transition-all duration-500 delay-200">
            {technologies.split(",").map((tech, index) => (
              <span
                className="inline-flex items-center gap-x-1.5 py-1 px-1 mx-1 text-xs font-light text-black dark:text-white bg-gray-200/80 dark:bg-zinc-500/30 hover:bg-white/30 dark:hover:bg-zinc-400/30 rounded-lg cursor-pointer border border-slate-200 dark:border-zinc-800"
                key={index}
              >
                <img
                  src={`/tech/${tech}.svg`}
