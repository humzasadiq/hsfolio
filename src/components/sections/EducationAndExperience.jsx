...
function EducationAndExperience() {
  useEffect(() => {
    ...
    if (carousel && window.HSCarousel) {
      if (carousel._hsCarousel) {
        carousel._hsCarousel.destroy();
      }
      try {
        new window.HSCarousel(carousel).init();
      } catch (error) {
        console.error("Error initializing carousel:", error);
      }
    } else {
      console.error("Carousel element or HSCarousel library not found.");
    }
  }, []);
    ...
    <section id="about" className="mt-20 mx-4 lg:mx-20 flex flex-col md:flex-row gap-4 md:gap-2">
      <div className="w-full md:w-[80%]">
        ...
        <h4 className="text-xl dark:text-white mt-6 font-bold flex gap-2 items-center">
          <PiCertificateFill className="text-2xl text-[#8364D8] dark:text-[#8364D8]" />
          Certificates
        </h4>
        <div
          data-hs-carousel='{
        ...
        >
                  ...
                  <div className="hs-carousel-slide relative" key={index}>
                    ...
                    <div className="flex justify-center items-center h-full bg-gray-100 p-6 dark:bg-neutral-900">
                      <div className="hs-tooltip [--placement:bottom] inline-block">
                        <a href={cert.link} target="_blank" rel="noreferrer" className="hs-tooltip-toggle text-center text-gray-800 dark:text-white hover:text-[#8364D8] hover:dark:text-[#8364D8]">
          ...
          <div className="hs-carousel-pagination flex justify-center absolute bottom-3 start-0 end-0 space-x-2">
            {user_info.certificates.map((cert, index) => {
              return (
                <span
                  className="hs-carousel-active:bg-[#8364D8] hs-carousel-active:border-[#8364D8] size-3 border border-gray-400 rounded-full cursor-pointer dark:border-neutral-600 dark:hs-carousel-active:bg-[#8364D8] dark:hs-carousel-active:border-[#8364D8]"
                  key={index}
                ></span>
              );
            })}
                ...
                <div className="flex gap-x-3 relative group rounded-lg">
                  ...
                  <div className="grow p-2 pb-8">
                      ...
                      {exp.descriptions.map((desc, index) => {
                        return (
                          <li className="flex space-x-3" key={index}>
                            <svg
                              className="flex-shrink-0 size-4 mt-0.5 text-[#8364D8] dark:text-[#8364D8]"
                              xmlns="http://www.w3.org/2000/svg"
                              width="24"
                              height="24"
                              viewBox="0 0 24 24"
                    ...
                    {exp.contact ? ( 
                      ...
                      <a href="mailto:humzasadiq52@gmail.com" target="_blank" rel="noreferrer" className="text-[#8364D8] dark:text-[#8364D8] mt-2">
                        email me 
                      </a>
                      <FaExternalLinkAlt className="text-[#8364D8] dark:text-[#8364D8] mt-2 text-sm"/>
                      </span>
                    ) : null}
                  </div>
                </div>
